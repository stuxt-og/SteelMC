...
impl Player {
    ...
    pub fn set_entity_flag(&self, flag: i8, value: bool) {
        ...
        let living_data = ent_data.living_entity_mut();

        let current = living_data.living_entity_flags.get();
    ...
    pub fn tick_using_item(&self) {
        ...
        let cached_item = self.use_item.lock().clone();

        let item_matches = {
            let inv = self.inventory.lock();
            let current = inv.get_item_in_hand(hand);
            current == &cached_item
        };
